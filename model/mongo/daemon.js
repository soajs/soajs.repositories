...
"use strict";
const colName = "daemons";
const core = require("soajs");
const Mongo = core.mongo;

let indexing = {};

function Daemon(service, options, mongoCore) {
	let __self = this;
	...
	if (indexing && !indexing[index]) {
		indexing[index] = true;
		//todo add indexes
		service.log.debug("Daemon: Indexes for " + index + " Updated!");
	}
}

Daemon.prototype.closeConnection = function () {
	let __self = this;
...
Daemon.prototype.update = function (data, cb) {
	let __self = this;
	if (!data ) {
...
Daemon.prototype.removeDaemon = function (data, cb) {
	let __self = this;
	if (!data || !data.id) {
...
Daemon.prototype.removeRepository = function (data, cb) {
	let __self = this;
