...
let lib = {
			...
			function startProcessing(soa, cb) {
				...
				async.series({
					...
					checkCanUpdate: function (callback) {
						catalogDriver.checkCanUpdate(data, catalog, models, callback);
					},
					...
					createCatalogBackWordCompatibility: function (callback) {
						catalogDriver.createCatalogBackWordCompatibility(catalog, soajs, models, callback);
					}
				}, () => {
					if (err) {
						return cb(err);
	...
	"updateGitRepo": (data, catalog, soajs, models, cb) => {
		let opts = {
			...
			"configSHA": data.branch.sha,
			"_id": data.repo._id,
			"active": true
		};
		if (data.swagger && data.swagger.sha){
			opts.swaggerSHA = data.swagger.sha;
		}
		if (data.soa && data.soa.sha){
			opts.configSHA = data.soa.sha;
		}
