...
let lib = {
				...
				if (soa.content.folders && Array.isArray(soa.content.folders) && soa.content.folders.length > 0) {
					async.eachSeries(soa.content.folders, function (file, callback) {
						...
						if (err) {
							...
							if (data.branch) {
								return cb(null, `Repository ${data.repo.repository} branch ${data.branch} is active!`);
							} else if (data.tag) {
								return cb(null, `Repository ${data.repo.repository} tag ${data.tag} is active!`);
							}
						}
					});
				} else {
	...
	"deleteCatalog_src": (soajs, opts, cb) => {
		soajs.awareness.connect('marketplace', function (res) {
			let options = {
				method: "delete",
				uri: "http://" + res.host + "/item/src",
				headers: res.headers,
				json: true,
				qs: {
	...
	"updateCatalog": (soajs, opts, cb) => {
		soajs.awareness.connect('marketplace', function (res) {
			let options = {
				"method": "put",
				uri: "http://" + res.host + "/item/" + opts.soa.type,
				headers: res.headers,
				json: true,
				body: {
					item: opts
