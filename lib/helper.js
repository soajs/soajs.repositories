		...
		for (let route in apiPath) {
			...
			methods.forEach(function (oneMethod) {
				...
				if (apiPath[route][oneMethod].responses && Object.keys(apiPath[route][oneMethod].responses).length > 0) {
					for (let errorCode in apiPath[route][oneMethod].responses) {
		...
		for (let route in apiPath) {
			...
			methods.forEach(function (oneMethod) {
				...
				if (apiPath[route][oneMethod].parameters && apiPath[route][oneMethod].parameters.length > 0) {
					for (let input in apiPath[route][oneMethod].parameters) {
						let oneInput = apiPath[route][oneMethod].parameters[input];
						if (oneInput['$ref']) {
		...
		if (data) {
			for (let key in data) {
				for (let subKey in data[key]) {
					if (data[key][subKey] && data[key][subKey].mv) {
